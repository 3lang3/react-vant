...
const TabsTitle = forwardRef<HTMLDivElement, TabsTitleProps>((props, ref) => {
  ...
  const renderText = () => {
    const Title = (
      <span
        className={clsx(bem('text', { ellipsis: !props.scrollable && props.type !== 'jumbo' }))}
        style={{ backgroundColor: props.type === 'capsule' && isActive && color }}
      >
        {(() => {
    ...
    const Description =
      props.type === 'jumbo' && !!props.description ? (
        <div className={clsx(bem('description'))} style={{ backgroundColor: isActive && color }}>
          {(() => {
