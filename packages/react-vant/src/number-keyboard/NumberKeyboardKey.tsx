...
const DeleteIcon = ({ bem }) => (
  <svg className={cls(bem('delete-icon'))} viewBox="0 0 32 22">
    <path
...
const NumberKeyboardKey: React.FC<NumberKeyboardKeyProps> = ({
  ...
  const onTouchEnd = (event) => {
    if (active) {
      ...
      props.onPress?.(props.text, props.type);
    }
  };

  const renderContent = () => {
    if (props.loading) {
      return <Loading className={cls(bem('loading-icon'))} />;
    }

    const text = children || props.text;
    switch (props.type) {
      case 'delete':
        return text || <DeleteIcon bem={bem} />;
      case 'extra':
        return text || <CollapseIcon bem={bem} />;
      default:
        return text;
    }
  };
