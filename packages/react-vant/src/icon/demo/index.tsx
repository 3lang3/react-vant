...
import { Cell, Grid, Space, Switch, Toast, Typography, Search } from 'react-vant';
...
export default () => {
  ...
  const baseIcons = React.useMemo(() => getTypeIcons('basic', keyword), [keyword]);
  const lineIcons = React.useMemo(() => getTypeIcons('outline', keyword), [keyword]);
  const fillIcons = React.useMemo(() => getTypeIcons('filled', keyword), [keyword]);
    ...
    <div className="demo-icon-inline">
      <Space align="center" className="demo-icon-inline--filter">
        <Cell
          center
          title="基础图标"
          rightIcon={<Switch checked={baseVisible} size={20} onChange={updateBaseVisible} />}
        />
        <Cell
          center
          title="线框风格"
          rightIcon={<Switch checked={lineVisible} size={20} onChange={updateLineVisible} />}
        />
        <Cell
          center
          title="实底风格"
          rightIcon={<Switch checked={fillVisible} size={20} onChange={updateFillVisible} />}
        />
        <Search
          value={keyword}
